...
`git clone https://github.com/ByteLegendQuest/java-fix-local-variable-scope`

A project directory named `java-fix-local-variable-scope` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-local-variable-scope` directory which is created just now.
